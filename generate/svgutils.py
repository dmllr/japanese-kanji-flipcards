...
def convert_path(d, canvas):
	if not d:
		return None
	path_normalized = normalise_svg_path(d)
	path = canvas.beginPath()
	points = []
	subpath_start = []
	lastop = ''

	for i in range(0, len(path_normalized), 2):
		op, nums = path_normalized[i:i + 2]

		# inverse y
		nums = [-n if p % 2 else n for p, n in enumerate(nums)]
		...
		# horizontal/vertical line absolute
			...
			points.extend([nums[0], points[-1]])
		elif op == 'V':
			path.lineTo(points[-2], nums[0])
			points.extend([points[-2], nums[0]])

		# horizontal/vertical line relative
		elif op == 'h':
			path.lineTo(points[-2] + nums[0], points[-1])
			points.extend([points[-2] + nums[0], points[-1]])
		elif op == 'v':
			path.lineTo(points[-2], points[-1] + nums[0])
			points.extend([points[-2], points[-1] + nums[0]])
		...
		# cubic bezier, absolute
			...
			points.extend([xi, yi, x2, y2, xn, yn])
		...
		# cubic bezier, relative
			...
			points.extend([xp + x1, yp + y1, xp + x2, yp + y2, xp + xn, yp + yn])
		elif op == 's':
			x2, y2, xn, yn = nums
			if len(points) < 4 or lastop not in {'c', 'C', 's', 'S'}:
				xp, yp, x0, y0 = points[-2:] * 2
			...
			points.extend([xi, yi, x0 + x2, y0 + y2, x0 + xn, y0 + yn])
		...
		# quadratic bezier, absolute
			...
			points.extend([x1, y1, x2, y2, xn, yn])
		elif op == 'T':
			if len(points) < 4:
				xp, yp, x0, y0 = points[-2:] * 2
			else:
